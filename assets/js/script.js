...
const wrapper = document.querySelector('.wrapper'),
    ...
    musicName = wrapper.querySelector('.song-deatils .name'),
    musicArtist = wrapper.querySelector('.song-deatils .artist'),
...
(showMoreBtn = wrapper.querySelector('#more-muscic')), (HideMusicBtn = musiclist.querySelector('#close'));
...
function nextMusic() {
    ...
    musicIndex++;
    //if musicIndex is greater than array length then musicIndx will be 1 so the first song will play
    musicIndex > allMusic.length ? (musicIndx = 1) : (musicIndex = musicIndex);
...
function prevMusic() {
    ...
    musicIndex--;
    //if musicIndex is less than 1 then musicIndx will be array length so the last song will play
    musicIndex < 1 ? (musicIndx = allMusic.length) : (musicIndex = musicIndex);
    loadMusic(musicIndex);
    playMusic();
    playingNow();
}
...
nextBtn.addEventListener('click', () => {
    nextMusic(); //calling next mucis function
});

//prev music btn event
prevBtn.addEventListener('click', () => {
    prevMusic(); //calling next mucis function
});

//update progress bar with according to music current time
mainAudio.addEventListener('timeupdate', e => {
    const currentTime = e.target.currentTime; //getting current time of song
    const duration = e.target.duration; //gettimg total duration time of  song
...
//let's update playing song current time according to the progress bar widh
...
repeatBtn.addEventListener('click', () => {
    ...
    //let's do different changes on different icon click using swith
    switch (getText) {
...
mainAudio.addEventListener('ended', () => {
    ...
    //let's do different changes on different icon click using swith
    switch (getText) {
        case 'repeat': //if this icon is repeat the simply we call the nextMusic function so the next song will play
            nextMusic();
            break;
        case 'repeat_one': //if  icon is repeat_one then we'll change the current playing song current time to 0 song will play from brginning
            mainAudio.currentTime = 0;
            loadMusic(musicIndex);
            playMusic(); //calling playMusic function
            break;
        case 'shuffle': //if icon is shuffle then change it to repeat
            //generating random index between the max range of affay length
...
function playingNow() {
    for (let j = 0; j < allLiTags.length; j++) {
        ...
        //let;s remove playing class from all other li expeect the last one which is clicked
        if (allLiTags[j].classList.contains('playing')) {
            ...
            audioTag.innerText = audioDuration; //passing t-duration value to audio duration innerText
        }
...
function clicked(element) {
    //getting li index of particulat clicked li tag
